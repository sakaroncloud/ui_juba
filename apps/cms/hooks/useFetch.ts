...
export const useLoadMoreFetch = (options: TLoadMore) => {
  return useInfiniteQuery({
    queryKey: options.imageName
      ? [options.queryKey, options.imageName]
      : [options.queryKey],
    ...
    getPreviousPageParam: (firstPage) =>
      firstPage?.meta?.hasPreviousPage
...
export const loadMore = async (
  ...
  try {
    const res = await fetch(
      BACKEND_URL +
        ENDPOINT +
        `?page=${pageParam}&take=${take}&search=${imageName}`,
      {
        headers: {
...
const getData = async <T>(options: TOption): Promise<T | null> => {
  ...
  const response = await fetch(
    BACKEND_URL +
      options.endPoint +
      (options.param ? `/${options.param + queryString}` : queryString),
    {
      headers: {
        Authorization: `Bearer ${session?.accessToken || ""}`,
      },
